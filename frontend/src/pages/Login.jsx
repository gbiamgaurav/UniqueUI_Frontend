  ...
  return (
    <div className="min-h-screen grid grid-cols-1 lg:grid-cols-2">
      <div className="hidden lg:flex flex-col justify-between p-10 border-r border-[var(--border)] bg-[var(--surface)]">
        ...
        <div>
          <h2 className="font-display text-5xl leading-tight">“Documents become<br/>dialogue.”</h2>
          <p className="mt-4 label-tag">— editorial of the machine, vol. 1</p>
        </div>
        <div className="label-tag">Plate 02 · Sign in</div>
      </div>
      ...
      <div className="flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center px-6 pb-12">
          <form onSubmit={submit} className="w-full max-w-md" data-testid="login-form">
            ...
            <p className="text-sm text-[var(--text-sub)] mt-2">Use email & password or continue with Google.</p>
            ...
            <input className="input-field mt-1" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} data-testid="login-email"/>

            <label className="label-tag mt-4 block">Password</label>
            <input className="input-field mt-1" type="password" required minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} data-testid="login-password"/>
            ...
            <p className="mt-6 text-xs text-[var(--text-sub)]">
              No account? <Link to="/signup" className="underline" data-testid="goto-signup">Create one</Link>
            </p>
          </form>
        </div>
      </div>
